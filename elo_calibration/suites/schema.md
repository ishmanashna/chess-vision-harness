...
| `k_factor` | ELO update sensitivity (default 48) |
| `initial_elo_non_stockfish` | Starting rating for harness rungs, inverse_sf, MinimalChess, `random`, etc. (default **500**) |

Stockfish tiers (`stockfish:0`–`20`) always start at their **catalog UCI ELO** and stay fixed (anchors).

**Stockfish handicaps** (`stockfish-handicap:*`, type `stockfish_harness`) use UCI skill 0 plus a play harness (`depth`, `movetime_ms`, `random_move_pct`). They start at **500** and are calibrated like other floaters — their catalog `elo` is only a matching hint.

## `pairs`

```yaml
pairs:
  - white: stockfish-handicap:noise10
    black: stockfish:0
    games: 20
...
```yaml
round_robin:
  opponents: [stockfish-handicap:noise10, stockfish-handicap:noise17, random]
  games_per_pair: 40
  colors: alternate
```
...
Example: `stockfish-handicap:noise10` (starts 500) beats `stockfish:0` (fixed 1320) → harness gains ELO; Stockfish stays 1320.
