...
export function createPlayBoard(mountEl, humanColor, onSubmitMove) {
  ...
  function applyInputState(canMove) {
    const want = !!canMove;
    if (want === inputEnabledRef.current) return;
    inputEnabledRef.current = want;
    if (want) {
      board.enableMoveInput(inputHandler, humanSide);
    } else if (!premove.getPremove()) {
      board.disableMoveInput();
    }
  }

  function applyPremoveInputState(canPremove) {
    premove.applyPremoveInputState(canPremove, inputEnabledRef);
  }
