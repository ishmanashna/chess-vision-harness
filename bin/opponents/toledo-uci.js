...
process.stdin.on('data', function(pData) {
	var _obj, _input, _line, _k;

	//-- Simplifies the input
	_input = pData.toString();
...
function _uciProcess(pLine)
{
	...
	switch (_tab[0].toLowerCase())
	{
		...
		case 'go':
		{
			...
			{
				_uciWrite('info score cp '+_cp+' depth '+_uciStrength+' time '+(_end-_start)+' nodes '+_nodes+' nps '+_nps+' pv 0000');
